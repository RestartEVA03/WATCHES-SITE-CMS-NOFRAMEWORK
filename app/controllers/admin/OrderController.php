...
use watchesshop\libs\Pagination;
...
class OrderController extends AppController
{
    public function indexAction(){
        ...
        $orders = \R::getAll("SELECT `order`.`id`, `order`.`user_id`, `order`.`status`, `order`.`date`, 
                    ...
                    ROUND(SUM(`order_product`.`price`), 2) AS `sum` FROM `order` JOIN `user` 
                        ON `order`.`user_id` = `user`.`id` JOIN `order_product` 
